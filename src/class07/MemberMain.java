...
/**
 * [미션1] 1. 다음과 같이 member[]배열에는 콤머로 구분된 한 회원에 대한 정보가 들어있다. 2. 콤머로 구분된 회원 데이터를
 * split 함수로 분리해서 각각의 회원 객체를 만드세요. 3. 생성된 회원 객체를 객체 배열에 담아서 출력하시오
 * 
 * split된 배열에 들어있는 값들로 회원 객체 생성해서 회원 객체 전용 배열에 저장
 */
public class MemberMain { // class s
	...
	public static void main(String[] args) { // main s
		...
		Member[] members = new Member[98]; // Member타입의 배열 members클래스 배열 객체 생성

		// 문자열 배열에 들어있는 값으로 객체 생성해서 members에 저장해주는 메소드 호출
		initData(members);

		// 1. 전체 회원 리스트 출력 메소드 만들기
		pirintMemberList(members);

		// [문제1] 20대후반 여성의 총 방문횟수 구해서 반환해주는 메소드 만들기
	...
	public static int getCount40(Member[] members) {
		int count = 0;
		for (Member member : members) {
			String ageTest = member.getAge().substring(0, 2); // 0을 포함하고 2는 포함하지 않음 숫자만 갖고온다.
	...
	public static void initData(Member[] members) {
		...
		for (int i = 0; i < member.length; i++) {
			String[] arr = member[i].split(","); // 콤머로 구분해서 주르고 배열에 입력
			int numVisit = Integer.parseInt(arr[3]);
			char gender = arr[4].charAt(0); // "남" -> '남' 으로 변경 (char타입)
			members[i] = new Member(arr[0], arr[1], arr[2], numVisit, gender, arr[5]); // 객체생성

//			System.out.println(
//					members[i].getName() + "\t" + members[i].getJoinDate() + "\t" + members[i].getLastVisitDate() + "\t"
//							+ members[i].getNumVisit() + "\t" + members[i].getGender() + "\t" + members[i].getAge());

		}
	}

	// 출력리스트
	public static void pirintMemberList(Member[] members) {
	...
	// 20대후반의 방문횟수 구하는 메소드
	public static int getVisitCount(Member[] members) {
		int count = 0;
